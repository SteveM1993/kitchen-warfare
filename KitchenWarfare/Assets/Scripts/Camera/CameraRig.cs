...
public class CameraRig : MonoBehaviour {
    ...
    void FollowTarget(Vector3 targetPos, Quaternion targetRotation)
    {
        if (!Application.isPlaying)
        {
    ...
    //Checks if player is hitting wall
    void CheckWall()
    {
        if (!pivot || !mainCamera)
        {
    ...
    //Moves camera forward when wall is hit
    ...
    //Positions camera to a give point
    ...
    //Hides mesh renderer when too close
    void CheckMeshRenderer()
    {
        ...
        float dis = Vector3.Distance(mainCamPos, targetPos + target.up);

        if (meshes.Length > 0)
        {
            for (int i = 0; i < meshes.Length; i++)
            {
                if (dis <= cameraSettings.hideMeshWhenDistance)
                {
